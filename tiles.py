...
class PlayerTile(DragBehavior, Image):
    ...
    def move_callback(self, obj, mouse):
        ...
        if self.collide_point(mouse.x, mouse.y):
            ...
            # checks the appropriate board space in glo.board
            ...
            if mouse.dx > 0 and x < glo.COLS-1:
                if glo.board[y][x+1] in glo.tiles.can_move['right']:
                    self.x += mouse.dx
            elif mouse.dx < 0 and self.x > 0:
                if self.x % glo.TILE_SIZE == 0:
                    if glo.board[y][x-1] in glo.tiles.can_move['left']:
                        self.x += mouse.dx
                else:
                    if glo.board[y][x] in glo.tiles.can_move['left']:
                        self.x += mouse.dx

            # VERTICAL MOVEMENT
            if mouse.dy > 0 and y < glo.ROWS-1:
                if glo.board[y-1][x] in glo.tiles.can_move['up']:
                    self.y += mouse.dy
            elif mouse.dy < 0 and self.y > 0:
                if self.y % glo.TILE_SIZE == 0:
                    if glo.board[y+1][x] in glo.tiles.can_move['down']:
                        self.y += mouse.dy
                else:
                    if glo.board[y][x] in glo.tiles.can_move['down']:
                        self.y += mouse.dy
